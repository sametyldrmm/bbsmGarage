...
export default function Kartlar() {
  ...
  const fetchYapilanlar = (id) => {
    if (Array.isArray(data)) {
      console.log(data);
    }
    console.log(data);
    fetch(`http://localhost:4000/${id}/yapilanlar`, requestOptionsPost)
  ...
  const fetchStokListesi = () => {
    ...
    const filtrelenmisKartlar = kartlar.filter(kart => 
      (kart.adSoyad != null && kart.markaModel != null && kart.plaka != null && kart.sasi != null && kart.km != null && kart.girisTarihi != null) &&  
